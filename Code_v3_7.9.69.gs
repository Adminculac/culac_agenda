...
/** บันทึก log การทำงานไว้ในชีตซ่อน "NotifyLog" เพื่อใช้ตรวจสอบว่าทำไมแจ้งเตือนไม่ทำงาน
 *  เปิดดูได้จาก Google Sheet โดยคลิกขวาที่แถบชื่อชีตด้านล่าง → "ยกเลิกซ่อน" (Unhide) */
function debugLog_(msg){
  try{
    var sh = sheet_('NotifyLog', ['เวลา','ข้อความ']);
    sh.getRange(sh.getLastRow()+1,1,1,2).setValues([[new Date().toISOString(), msg]]);
  }catch(e){ /* ห้าม throw จาก log เด็ดขาด */ }
}
...
function doPost(e){
  try{
    ...
    try{
      ...
      debugLog_('doPost เรียกทำงาน · จำนวนโครงการที่ส่งมา = ' + ids.length + (ids.length?(' · id: '+ids.join(', ')):''));
      if(ids.length){
        ...
        ids.forEach(function(id){
          ...
          debugLog_('ตรวจโครงการ ' + id + ' · สถานะเดิม=' + oldStatus + ' · สถานะใหม่=' + (v&&v.status) + ' · ลบ=' + del);
          if(!del && v && v.status){
            if(v.status==='submitted' && oldStatus!=='submitted'){ notify.push({type:'submitted', project:v}); debugLog_('→ เข้าเงื่อนไขแจ้งเตือน: มีโครงการใหม่ (' + (v.code||id) + ')'); }
            if(v.status==='running' && oldStatus!=='running'){ notify.push({type:'approved', project:v}); debugLog_('→ เข้าเงื่อนไขแจ้งเตือน: อนุมัติแล้ว (' + (v.code||id) + ')'); }
          }
          if(rowOf[id]!==undefined){
            var ri = rowOf[id];
      ...
      lock.releaseLock();
      debugLog_('สรุป: จำนวนที่จะแจ้งเตือน = ' + toNotify.length);
      if(toNotify.length){
        try{ sendNotifications_(toNotify); debugLog_('ส่งอีเมลแจ้งเตือนสำเร็จ (' + toNotify.length + ' ฉบับ)'); }
        catch(notifyErr){ debugLog_('ส่งอีเมลล้มเหลว: ' + String(notifyErr)); }
      }
      return jsonOut_(result);
    } catch(errInner){ try{ lock.releaseLock(); }catch(_){ } debugLog_('เกิดข้อผิดพลาดใน doPost: ' + String(errInner)); throw errInner; }
  }catch(err){ debugLog_('เกิดข้อผิดพลาดนอก lock: ' + String(err)); return jsonOut_({ ok:false, error:String(err) }); }
}
